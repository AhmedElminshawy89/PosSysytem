...
const TodayOrder = () => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`<td colspan="7" style="text-align: center;">Total:</td>`);
  ...
  const calculateTotal = () => {
    return data.reduce((total, row) => total + parseFloat(row[7]), 0).toFixed(2);
  };
                ...
                data.map((row, index) => (
                  <tr key={index} className="fs-5">
                    {row.map((cell, cellIndex) => (
                      columns[cellIndex].visible && <td key={cellIndex}>{cellIndex === 8 ? renderActionIcons() : cell}</td>
                    ))}
                  </tr>
                ))
              )}
